...
class Fog_panelGameBoard extends panelGameBoard
{
	...
	function mapHTML() {
		...
		if (!(file_exists($newFile)))
			$newFile='variants/'.$this->Variant->name.'/resources/fogmap.php?&verify='.$verify.'&gameID='.$this->id.'&turn='.$mapTurn;
		$oldMap="map.php?";
		$newMap="variants/".$this->Variant->name."/resources/fogmap.php?verify=".$verify."&";
	...
	protected function mapJS($mapTurn) {
		...
		libHTML::$footerIncludes[] = '../variants/'.$this->Variant->name.'/resources/my_mapUI.js';
	}
...
class PunicWarsVariant_panelGameBoard extends Fog_panelGameBoard {}
